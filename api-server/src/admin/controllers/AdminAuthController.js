...
class AdminAuthController {
    ...
    static async signin(req, res) {
        console.log('here');
        const user = await AdminUserModel.findOne({email: req.body.email});
        if (!user) {
    ...
    static async signup(req, res) {
        const isAlreadyUser = await AdminUserModel.findOne({email: req.body.email});
        if (isAlreadyUser) {
        ...
        MailService.sendWithTemplate(
            ...
            {
                template: 'admin/adminSingup',
                data: {
                    link: `${config.adminFrontendHost}/setpassword/${user._id.toString()}/${token}`
                }
            }
        );
    ...
    static async setPassword(req, res) {
        const user = await AdminUserModel.findOne({_id: req.body.id});
        if (!user) {
    ...
    static async renewPasswordToken(req, res) {
        ...
        MailService.sendWithTemplate(
            ...
            },
            {
                template: 'admin/adminSingup',
                data: {
                    link: `${config.adminFrontendHost}/setpassword/${user._id}/${token}`
                }
            }
        );
        res.json({status: 'success'});
    ...
    static async refreshTokens(req, res) {
        ...
        const user = await AdminUserModel.findOne({_id: verifyData.id});
    ...
    static async logout(req, res, next) {
        const user = await AdminUserModel.findOne({_id: req.userId});
        if (!user) {
    ...
    static async restorePassword(req, res, next) {
        ...
        MailService.sendWithTemplate(
            ...
            {
                template: 'admin/adminRestorePassword',
                data: {
                    host: config.adminFrontendHost,
